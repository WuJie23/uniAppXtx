...
const getGoodData = async (id: string) => {
  ...
  uni.setNavigationBarTitle({
    title: goodsData.value!.name,
  })
  console.log(goodsData, 'res')
}
//
const mainPicturesIndex = ref(1)
const getMainPicturesIndex = (event: any) => {
  console.log(event.detail, ' event.detail')
  mainPicturesIndex.value = event.detail.current + 1
}
onLoad((option: any) => {
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      <!-- 商品主图 -->
      <view class="preview">
        <swiper circular @change="getMainPicturesIndex">
          <swiper-item v-for="(item, index) in goodsData?.mainPictures" :key="index">
            <image mode="aspectFill" :src="item" />
          </swiper-item>
        </swiper>
        <view class="indicator">
          <text class="current">{{ mainPicturesIndex }}</text>
